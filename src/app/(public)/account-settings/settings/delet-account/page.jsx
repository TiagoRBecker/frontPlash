...
const DeletAcc = () => {
  const router = useRouter()
   ...
   const onsubmit = async (e)=>{
    ...
    try {
      const user = await ApiHook.deleteAccount(password)
      setLoading(true)
    } catch (error) {
  ...
  return (
    <section className="w-full h-screen flex  flex-col px-10   gap-5  ">
      ...
      <form className="flex flex-col item gap-4 py-4 w-full" onSubmit={onsubmit}>
        <p>
        ...
        <div className="flex items-center h-full w-full gap-2">
          <div className="flex flex-col w-full gap-2">
            ...
            />
          {error && <p className="text-red-500">Senha Inválida</p>}
          </div>
        </div>
      ...
      <div className="w-full flex flex-col gap-5 bg-[#14b7a1]  text-white rounded py-2 px-2">
        ...
        <div className="flex gap-2">
          ...
          <Link href={"/account-settings/settings/edit-perfil"}>
            <button className="w-full md:w-[250px] bg-black px-4 py-2 rounded-md text-white">
              Perfil
            </button>
          </Link>
        </div>
